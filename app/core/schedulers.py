...
async def predict_biweekly_contest():
    ...
    if passed_weeks % 2 != 0:
        logger.info(f"predict_biweekly_contest return, passed_weeks={passed_weeks} is odd for now={utc}")
        return
    contest_name = f"biweekly-contest-{passed_weeks // 2 + BIWEEKLY_CONTEST_BASE.num}"
    logger.info(f"predict_biweekly_contest running, contest_name={contest_name}")
...
async def predict_weekly_contest():
    ...
    logger.info(f"predict_weekly_contest running, contest_name={contest_name}")
    await asyncio.sleep(15)
